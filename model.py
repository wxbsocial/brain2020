...
class _FCN(nn.Module):
    ...
    def dense_to_conv(self):
        ...
        # fcn.classifier[1] = nn.Conv3d(160, 30, 6, 1, 0).cpu()
        fcn.classifier[1] = nn.Conv3d(8*self.num, 30, 6, 1, 0).cpu()
        fcn.classifier[4] = nn.Conv3d(30, 2, 1, 1, 0).cpu()
...
class _MLP_COPD(nn.Module):
    "MLP that only use age gender MMSE"
    def __init__(self, in_size, drop_rate, fil_num):
        super(_MLP_COPD, self).__init__()
