import os
import numpy as np
from model import _CNN, _FCN, _MLP_A, _MLP_B, _MLP_C, _MLP_D,_MLP_COPD
from utils import matrix_sum, get_accu, get_MCC, get_confusion_matrix, write_raw_score, DPM_statistics, timeit, read_csv
from dataloader import CNN_Data, FCN_Data, MLP_Data, MLP_Data_apoe, CNN_MLP_Data,MLP_Data_COPD
import torch
...
class CNN_Wrapper:
    def __init__(self,
        ...
        self.model = _CNN(fil_num=fil_num, drop_rate=drop_rate).cpu()
    ...
    def train(self, lr, epochs):
        ...
        self.criterion = nn.CrossEntropyLoss(weight=torch.Tensor([1, self.imbalanced_ratio])).cpu()
...
class FCN_Wrapper(CNN_Wrapper):
    ...
    def __init__(self, fil_num,
        ...
        self.model_name = model_name
        self.eval_metric = get_accu if metric == 'accuracy' else get_MCC
        self.model = _FCN(num=fil_num, p=drop_rate).cpu()
        self.prepare_dataloader(batch_size, balanced, Data_dir)
...
class MLP_Wrapper_COPD:
    ...
    def prepare_dataloader(self, batch_size, balanced):
        ...
        valid_data = MLP_Data_COPD(self.exp_idx, stage='valid', seed=self.seed)
        test_data  = MLP_Data_COPD(self.exp_idx, stage='test', seed=self.seed)
        sample_weight, self.imbalanced_ratio = train_data.get_sample_weights()
        # the following if else blocks represent two ways of handling class imbalance issue
        if balanced == 1:
            # use pytorch sampler to sample data with probability according to the count of each class
    ...
    def test(self, repe_idx):
        ...
        with torch.no_grad():
            for stage in ['train', 'valid', 'test']:
                data = MLP_Data_COPD(self.exp_idx, stage=stage, seed=self.seed)
                dataloader = DataLoader(data, batch_size=10, shuffle=False)
                f = open(self.checkpoint_dir + 'raw_score_{}_{}.txt'.format(stage, repe_idx), 'w')
                matrix = [[0, 0], [0, 0]]
                for idx, (inputs, labels) in enumerate(dataloader):
    ...
    def draw(self, repe_idx):
        ...
        for i in range(n_classes):
            a,p = self.cal_actuals_and_probabilities(i)
            actuals.append(a)
            probabilities.append(p)


        self.plot_roc(actuals, probabilities, n_classes)
  

    def cal_actuals_and_probabilities(self, n_class):

        self.optimal_epoch = 990
        ...
        with torch.no_grad():
            for stage in ['test']:
                data = MLP_Data_COPD(self.exp_idx, stage=stage, seed=self.seed)
                dataloader = DataLoader(data, batch_size=1000, shuffle=False, drop_last=False, pin_memory=True, num_workers=1)
                #matrix = [[0, 0], [0, 0]]
                for idx, (inputs, labels) in enumerate(dataloader):
                    inputs, labels = inputs, labels
                    ...
                    y_prob = nn.Softmax(dim =1)(outputs)[:,n_class].numpy()
                    probabilities.extend(y_prob)


        return actuals, probabilities
    
    def plot_roc(self, actuals,  probabilities, n_classes):
        """
        ...
        roc_auc = dict()
        for i in range(n_classes):
            fpr[i], tpr[i], _ = roc_curve(actuals[i], probabilities[i], pos_label = i)
