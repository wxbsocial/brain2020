...
from utils import PatchGenerator, padding, read_csv, read_csv_complete, read_csv_complete_apoe, get_AD_risk,read_csv_copd
import random
import pandas as pd
import csv
...
class FCN_Data(CNN_Data):
    ...
    def __getitem__(self, idx):
        ...
        if self.stage == 'train' and not self.whole:
            ...
            data = padding(data, win_size=self.patch_size // 2)
...
class MLP_Data_COPD(Dataset):
    def __init__(self, exp_idx, stage, seed=1000):
        random.seed(seed)
        self.exp_idx = exp_idx
        ...
        if stage in ['train', 'valid', 'test']:
            ...
            self.path = './lookupcsv/{}.csv'.format(stage)
        print('path:', self.path)
        self.Data_list, self.Label_list, self.demor_list = read_csv_copd(self.path)
        # for filename in self.Data_list:
        #     print('filename:', filename)
    ...
    def __getitem__(self, idx):
        label = self.Label_list[idx]
        demor = self.demor_list[idx]
        return np.asarray(demor).astype(np.float32),label
